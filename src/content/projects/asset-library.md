...
description: "정적 에셋 라이브러리 V1의 초기 구축부터 V2 API 런타임 설계·구현까지 담당했습니다. 에셋 생성·다운로드 검증을 기반으로, 목록 조회와 전달 정책을 분리하고 파일·메타데이터의 버전 일관성을 확보했습니다."
...
Shotloom에서 사용할 에셋을 생성하고 검증해 전달하는 독립 라이브러리입니다. **로컬 정적 라이브러리 V1을 처음 구축하고, 이후 팀에서 정의한 API 계약을 바탕으로 V2 런타임을 설계·구현했습니다.**
